<?php
	...
	$code4 = isset($_GET['code4'])?$_GET['code4']:"212";
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	$titre_document = "$tag Correction";
	$repertoire_rep = "./files/$classe/_Copies/_Sujets/$tag/rep$code4";
...
<html>
	...
	<body>
			<table>
				<tr id="sommaire">
					<td><font size="+4"><?php echo($titre_document);?></font></td>
				</tr>
			</table>
<?php
	...
	if(file_exists($sujet2DS)){
		...
		while(!feof($fp)){
			...
			if($part[0]=="Q") {//Question
				...
				$lettre_elv = "</td><td width=\"20px\"><img src=\"./icon/A.gif\" title=\"Q$i\">";
		...
		echo("<center>Pas de fichier  $sujet2DS !!</center>");
	}
